...
<div id="calculadora">
	...
	<div id="planta">
		...
		<div id="cuerpo">
			...
			<label id='factorp'>Factor de conversion DC/AC:</label><input id='factorp' value="0.77"/><br><hr>
...
function mostrarGraficaTabla(datos){
	...
	var options = {
		...
		//yaxix:  [{ min:0, position:'left', label:'[kWh]'},{min:0, position:'right', label:'[k]'}],
		y1axix:  { min:0, position:'left', label:'[kWh]'},
		y2axis: { min:0, position:'right', label:'[kWh/m2/dia]'},
	};
	var plot = jQuery.plot(placeholder, datos, options);
	var titulo = '<label id="grafico1">Radiación solar y Energia producida en un año</label>';
	return titulo;
}
...
function mostrarGraficaHoraria(datos){
	...
	var options = {
		series: { lines: { show: true, shadowSize: 0 }},
		legend: { show: true, noColumns: 12, position:'se'},
		xaxis:  { max:24,min:0,ticks: 24, minTickSize: 1, tickDecimals:0 },
		yaxix:  { min:0, label:'[Wh]'},
	};
	var plot = jQuery.plot(placeholder, datos, options);
	var titulo = '<label id="grafico2">Radiación solar horaria para un dia del mes</label>';
	return titulo;
}

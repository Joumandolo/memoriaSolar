...
$RGHd = array(0,8647,7389,7212,5486,4521,3889,4306,5059,6146,7678,8264,8862);
...
for($i = 0; $i < 12; $i++){
	$energiaSolarT = round(energiaSolarTotal($alb,$incl,$ori,$RGHd[$i+1],$l,$nda[$i],$cs,$hd),4);
	$energiaProducida = round(energiaProducida($ap,$eps,$fp,$energiaSolarT,$ndm[$i]),4);
	$energiaValor = round($energiaProducida * $vkwh,4);

	$r = $r + $energiaSolarT;
	$e = $e + $energiaProducida;
	$c = $c + $energiaValor;

	$filaMes = $filaMes.'<th>'.$ma[$i].'</th>';
	$filaRadiacion = $filaRadiacion.'<td id="val">'.$energiaSolarT.'</td>';
	$filaEnergia = $filaEnergia.'<td id="val">'.$energiaProducida.'</td>';
	$filaCosto = $filaCosto.'<td id="val">'.$energiaValor.'</td>';
}
echo "	<table>
